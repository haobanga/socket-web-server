...
class User(Model):
    ...
    def __init__(self, form):
        ...
        self.role = form.get('role') or 10
    
    def hashmd5_salt(self, pwd, salt="?>^32$%sd"):
        from hashlib import md5
        def hash_md5(b):
            return md5(b.encode("ascii")).hexdigest()
        m = hash_md5(hash_md5(pwd) + salt)
        return m
    ...
    def validate_login(self):
        ...
        #         return True
        # return False
        return u is not None and u.password == self.hashmd5_salt(self.password)
        # 这样的代码是不好的，不应该用隐式转换
        # return u and u.password == self.password
         
    def update_password(self, pwd):
        self.password = self.hashmd5_salt(pwd)
        self.save()
        
    def validate_register(self):
        u = User.find_by(username=self.username)
        if u is None:
            self.password = self.hashmd5_salt(self.password)
            self.save()
            return self
